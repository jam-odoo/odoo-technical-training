...
class Session(models.Model):
    ...
    duration = fields.Float(string='Duration in Days', digits=(6, 2), help='Duration in days', default=1)
    ...
    level = fields.Selection(string='Level', related='course_id.level', readonly=True)
    responsible_id = fields.Many2one(string='Responsible', related='course_id.responsible_id', readonly=True, store=True)
    description = fields.Html(string='Description')
    percentage_per_day = fields.Integer('%', default=100)
    attendees_count = fields.Integer(string='Attendees count', compute='_get_attendees_count', store=True)
    color = fields.Integer(string='Color Index')
    exclude = fields.Boolean(string='Planning Exclude')
